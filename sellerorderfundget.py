...
params = {
    ...
    "data_select": "fund_serviceFee,fund_fundPay",  # Adjust based on your requirements
}
...
# Log file names
log_file = f"{LOG_DIR}seller_order_fund_get_logs_{time.strftime('%Y-%m-%d_%H-%M-%S')}.json"
error_log_file = f"{LOG_DIR}seller_order_fund_get_error_{time.strftime('%Y-%m-%d_%H-%M-%S')}.json"

try:
    # Make the API request
    seller_order_fund_get_request = requests.get(url, params=params)
    seller_order_fund_get_response = seller_order_fund_get_request.json()

    # Log API request
    with open(log_file, "w") as f:
        json.dump({
            "request_params": remove_sensitive_info(params),
            "response": seller_order_fund_get_response,
        }, f, indent=4)

    # Example of handling the response data
    if seller_order_fund_get_response.get("error_response"):
        with open(error_log_file, "w") as f:
            json.dump({
                "request_params": remove_sensitive_info(params),
                "response": seller_order_fund_get_response,
            }, f, indent=4)
        print(f"Error: {seller_order_fund_get_response['error_response']['msg']}")
    else:
